    ...
    suggestions.forEach(suggestion => {
        ...
        if (suggestion.type === 'history') {
            ...
            removeBtn.addEventListener('click', async (e) => {
                ...
                await storage.removeSearchFromHistory(suggestion.text);
...
export function initializeSettingsUI() {
    ...
    // Hava durumu efektleri ayarını yükle
    const weatherEffectsEnabled = storage.getStoredJSON('weatherEffectsEnabled', true); // Varsayılan olarak açık
    dom.weatherEffectsToggle.checked = weatherEffectsEnabled;

    // Hava durumu debug ayarını yükle
    const debugEffect = storage.getStoredJSON('debugWeatherEffect', 'auto');
    dom.debugWeatherEffectSelect.value = debugEffect;
